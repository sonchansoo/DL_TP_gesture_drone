...
def main_oakd():
    ...
    cam_rgb.setPreviewSize(640, 480)  # 미리보기 해상도 설정
    ...
    try:
        with dai.Device(pipeline) as device:
            ...
            if not video_writer.isOpened():
                print(f"WARNING: '{output_filename}' 파일을 열지 못했습니다. 녹화가 저장되지 않습니다.")

            print("'q' 키를 누르면 녹화 및 화면 표시를 종료합니다.")
            ...
            while True:
                ...
                if cv2.waitKey(1) == ord('q'):
                    print("종료 키 입력됨. 녹화를 중단하고 프로그램을 종료합니다.")
                    break

            # 비디오 라이터 릴리즈
            if video_writer.isOpened():
                video_writer.release()
                print(f"녹화 파일 '{output_filename}' 이(가) 저장되었습니다.")
